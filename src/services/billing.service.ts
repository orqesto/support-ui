...
const getBillingRecords = (page = 1, limit = 20, anomalyOnly = false) =>
  apiClient
    .get<PaginatedResponse<BillingRecord>>('/billing/records', {
...
const getPaymentRegistry = (page = 1, limit = 20) =>
  apiClient
    .get<PaginatedResponse<RegistryEntry>>('/billing/registry', {
      params: { page, limit },
    })
    .then((r) => r.data);

const getAgingReport = () =>
  apiClient.get<{ buckets: AgingBucket[] }>('/billing/aging').then((r) => r.data);
