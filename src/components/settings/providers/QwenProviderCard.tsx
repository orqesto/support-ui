...
export const QwenProviderCard = ({
  ...
  onSave,
  onCancel,
}: QwenProviderCardProps) => {
  const [showForm, setShowForm] = useState(false);
  const [config, setConfig] = useState<QwenConfig>({
    apiKey: '',
    baseUrl: '',
  ...
  const handleEdit = (integration: Integration) => {
    setConfig(integration.config as QwenConfig);
    setShowForm(true);
    onEdit(integration);
  };
      ...
      <CardHeader>
        <div className="flex justify-between items-center gap-3">
          ...
          >
            ...
            {integrations.length > 0 ? 'Update' : 'Add'} Qwen
          </Button>
        </div>
      </CardHeader>
        ...
        {showForm && (
          <div className="p-4 space-y-4 rounded-lg border bg-muted/50">
            <h4 className="font-medium">
              {editingId ? 'Edit Qwen Configuration' : 'Add Qwen Configuration'}
            </h4>
