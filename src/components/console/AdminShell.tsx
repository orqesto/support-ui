      ...
      >
        ...
        <nav className={cn('overflow-y-auto flex-1 p-3 space-y-1', collapsed && 'px-2')}>
          ...
          <div className="pb-2 mb-2 space-y-1 border-b border-border">
            ...
            {!isPlatform && isAdmin && (
              <NavTip label="Platform console">
                <NavLink
                  to="/console/platform/alliances"
                  aria-label={collapsed ? 'Platform console' : undefined}
                  className={cn(
                    'flex gap-2 items-center px-3 py-2 w-full text-sm rounded-md text-muted-foreground hover:bg-accent hover:text-foreground',
                    collapsed && 'justify-center px-0'
                  )}
                >
                  ...
                  {!collapsed && <span>Platform console</span>}
                </NavLink>
              </NavTip>
            )}
          </div>
          {sections.map((section) => {
            ...
            return (
              <NavTip key={section.id} label={section.label}>
                <NavLink
                  to={to}
                  end={section.index}
