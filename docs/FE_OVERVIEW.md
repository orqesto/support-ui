...
- **Message detail v3 layout (2026-09-22, #454)** — slide-over: header (icon row with key tooltips, identity + compact received-at, chips + Assign to me + split Resolve, Dept/Assigned/Category/Labels value row), state strip under the header (tinted: red filtered/spam, amber suspicious, neutral resolved/binned), tab strip, thread, composer. Full page ≥1024px (`useMediaQuery`): two columns — main as above minus the meta row and tabs; a 312px sidebar gets the meta block as rows (portal from `MessageDetailHeader` via `metaTarget`) and `MessagePanelTabs variant="sidebar"` (no Thread tab, always open, never switches the composer mode). Narrower full page keeps the slide-over layout.
