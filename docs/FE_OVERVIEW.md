...
- **`ResolveSplitButton` + `resolveMode.ts`** — The resolve DECISION, in the message-detail header (message detail v3, 2026-09-22). One predicate (`getResolveMode` → `'unreviewed' | 'active' | null`) decides for BOTH the header and the strip, so it can never show twice or not at all. One press opens the same confirm the old footer did (`noKbResolveDialog`: reject/`markAsProcessed` for unreviewed, close-confirm for active — both end `closed`); the caret holds Resolve & save to KB (active only) · Not customer work · Resolve & move to spam. "Resolve & move to spam" sends `move_to_spam` + `confirm` = CONFIRMED spam (owner, 2026-09-22: what our filters bin is unconfirmed; an agent resolving as spam confirms it — supersedes the SP-D5 wording rule for this control; the thread lands in Spam (confirmed), not Resolved). The More ("…") menu has no Move to Spam. Every AGENT path to spam is confirmed: the caret, Needs Routing's and the suspicious banner's "Resolve & move to spam", and a kanban drag onto Spam (`dndConfirmsSpam`). Only the filters produce unconfirmed spam, which the filtered banner's "Confirm — it is spam" (`confirm_spam`) confirms.
- **`detailShortcuts.ts`** — R reply · N note · E (opens the resolve dialog) · J/K (threads view only, `neighbourThread` skips spam-log rows, no wrap) · Esc (slide-over only, prompt-aware close). `shortcutFor` is pure; guards: inputs, contenteditable, modifiers, IME, `defaultPrevented`, any open `[role=dialog|alertdialog|menu]`. `shortcutHint` builds the composer hint from the same context.
- **`threadDays.ts`** — day separators; `threadTimeOf` is the ONE ordering rule for the sort and the separators. ⚠️ it prefers an inbound `sentAt` (sender's Date header) while `ThreadMessageItem` displays `receivedAt` — a bubble can show a different day than its separator when a sender's clock is off.
- **Message detail v3 layout (2026-09-22, #454)** — slide-over: header top to bottom — icon row with key tooltips; the DECISIONS (Assign to me + split Resolve) on their own right-aligned line, always there, never among the chips (#462 — on the chip row they jumped lines with the chip count; not rendered when there is no decision); subject; identity + compact received-at; chips; Also-matched banner; Dept/Assigned/Category/Labels value row, state strip under the header (tinted: red filtered/spam, amber suspicious, neutral resolved/binned), tab strip, tab badges: Files, Notes, KB (= the suggested options + KB references the tab lists, tagged per thread so a switch never shows the previous thread's number), and a DOT on Customer when a lookup is available and the customer has an email identity (availability is per workspace+user, so a number would read the same on every thread — owner, 2026-09-22), thread, composer. Full page ≥1024px (`useMediaQuery`): two columns — main as above minus the meta row and tabs; a 312px sidebar gets the meta block as rows (portal from `MessageDetailHeader` via `metaTarget`) and `MessagePanelTabs variant="sidebar"` (no Thread tab, always open, never switches the composer mode). Narrower full page keeps the slide-over layout.
- **Thread (v3)** — `ThreadMessageItem`: incoming = card on the canvas, reply = soft `--agent` ground (agent plain text is NOT prose-inverted), mono meta line with the name in the display face, attachments inside the bubble. `ThreadNoteItem`: internal notes inline at the time written (amber), merged into the timeline in `MessageDetail`; day separators group the merged list. `MessageGhostBubble`: dashed teal AI row.
- **Composer (v3)** — Reply / Internal note tabs + key hint; each editor keyed by mode (sharing one instance kept the first placeholder); `RichTextEditor transparent` lets the amber note field show.
- **`useAssignToMe`** — header "Assign to me" (`conv_<id>`); a department refusal is surfaced via `getApiErrorMessage`.
- **`MessageActionStrip`** — State BANNERS only since message detail v3 (filtered / spam-flagged / suspicious / resolved / closed); the unreviewed and active decisions moved to `ResolveSplitButton`. Filtered and suspicious states are taxonomy-aware: phishing/scam show a red "Quarantined · [threat]" label with "Not a Threat — Approve" (amber styling) and no "Move to Spam" option; transactional shows "Auto-archived · System email"; spam/promotional show the default filtered label with approve only. Uses `getFilteredCategoryMeta()` from `messageHelpers`.
